...
def custom_draw(results, image):
    ...
    if results.keypoints is not None:
        connections = [(0,1), (1,2), (2,3), (3,4),(0,3),(1,4)]
        min_points_required = max(e for pair in connections for e in pair) + 1

        for kps in results.keypoints.xy.cpu().numpy():
            # 添加有效性检查
            if len(kps) == 0 or kps.shape[0] < min_points_required:
                continue  # 跳过无效关键点

            # 绘制关键点（带坐标有效性校验）
            valid_points = []
            for i, (x,y) in enumerate(kps):
                if x >=0 and y >=0:  # 忽略负坐标
                    color = (0, 255, 0) if i < 5 else (0, 0, 255)
                    cv2.circle(img, (int(x), int(y)), 5, color, -1)
                    cv2.putText(img, str(i), (int(x)+5, int(y)), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
                    valid_points.append((x,y))
                else:
                    valid_points.append(None)  # 标记无效点
    ...
    return cv2.cvtColor(img, cv2.COLOR_BGR2RGB) # 保持RGB格式
        ...
        with gr.Row():
            with gr.Column():
                ...
                model_id = gr.Dropdown(
                    label="Model",
                    choices=[
                        "/root/Projects/rm/yolov12/runs/pose/train38/weights/best.pt",
                        "/root/Projects/rm/yolov12/runs/pose/train39/weights/best.pt",
                        "/root/Projects/rm/yolov12/runs/pose/train39/weights/last.pt",
                        "/root/Projects/rm/yolov12/runs/pose/train38/weights/last.pt",
                        "yolov12x.pt",
                    ],
                    value="/root/Projects/rm/yolov12/runs/pose/train39/weights/last.pt",
                )
